...
- `save_level_data`: Saves the data of all `StateMachine`, `CharacterEntity` and `PlayerEntity` nodes present in the current level. This function is called automatically by any `Transfer` node when leaving the current level.
...
- `load_level_data`: Loads the data of all `StateMachine`, `CharacterEntity` and `PlayerEntity` nodes present in the current level. This function is called automatically by any `Level` node when entering a level.
...
<Callout type="info">
  In your project, you can call the save and load functions as needed to suit
  the specific requirements of your game, or leave the system as is to handle
